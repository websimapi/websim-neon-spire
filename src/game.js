...
export class Game {
    ...
    start() {
        ...
        // Start Recording
        this.recorder.start(this.nodes);
        this.recordTimer = 0;
    ...
    input() {
        ...
        if (this.player.state === 'orbit') {
            ...
            audio.play('dash');
            this.recorder.recordEvent('audio', 'dash');
        }
    }
    ...
    addNode(x, y, type) {
        ...
        if (this.running) {
             this.recorder.recordEvent('addNode', {x, y, type});
        }
    }
    ...
    handleCollision(node) {
        ...
        if (node.type !== 'start') {
            ...
            audio.play('hit');
            this.recorder.recordEvent('audio', 'hit');
    ...
    gameOver() {
        this.running = false;
        audio.play('explode');
        this.recorder.recordEvent('audio', 'levelup'); // Reuse levelup for end sound
        const replayData = this.recorder.stop();
    ...
    loop(timestamp) {
        ...
        // Recording logic - Capture at ~30 FPS
        this.recordTimer += dt;
        if (this.recordTimer >= 0.033) {
            this.recorder.recordFrame(this.player, this.cameraY);
            this.recordTimer = 0;
        }
